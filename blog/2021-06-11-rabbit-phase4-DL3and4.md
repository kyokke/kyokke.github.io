 ...
 - BPTT (Back Propagation Through time)
   ...
   - 更新式の導出 その 1. ( 実装面, 多次元考慮)  
     ...
     - 以上を考慮して、各パラメータの更新式は下記の通りとなる (下のコード演習問題相当のアルゴリズムが導出できた)
       ...
       - 本更新式は、以下のコード演習問題の bptt()関数相当の式となっている。また、講義動画及び資料p. の更新式も同じものを指している認識
         - 資料中 p. の  $\delta^{k}$  すなわち $\frac{\partial E}{\partial u^{t-k}} は、$\delta^{t,t-k}$ すなわち $\frac{\partial E^t}{\partial u^{t-k}}$ に差し替えられるべきである。
         - 一方、それよりも前の p. については、正しく $\delta_{t}$ について議論がされている
   - コード演習問題
     - ~~~
<figure style="text-align:center;">
...
~~~
   ...
   - 更新式の導出 その 2. 
     - その1. で導出された式は、simple_RNN のコード例で与えられた更新式とは異なる。これは以下のように、更新式1 からさらに 重複計算を取り除くように変形をすることで求められる。
       - 講義動画中で主に議論され導出を目指していた(と思われる)更新式もこちらである。
     - 
